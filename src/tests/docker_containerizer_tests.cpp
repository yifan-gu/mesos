...
TEST_F(DockerContainerizerTest, DOCKER_Usage)
{
  ...
  // Wait for at most 5 seconds to get the cpu time measurements.
  Future<ResourceStatistics> usage;
  bool cputimeObserved = false;
  ...
  while (elapsed < Seconds(5)) {
    ...
    if (usage.get().cpus_user_time_secs() > 0 &&
        usage.get().cpus_system_time_secs() > 0) {
      VLOG(2) << "utime:" << usage.get().cpus_user_time_secs();
      VLOG(2) << "stime:" << usage.get().cpus_system_time_secs();
      cputimeObserved = true;
      break;
    }
